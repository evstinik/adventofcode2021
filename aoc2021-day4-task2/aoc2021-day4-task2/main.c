...
int main() {
    short input[MAX_SEQ], input_len = 0, n, max_move_per_col[GRID_SIZE], max_move_per_row[GRID_SIZE], min_move_per_grid = N, global_max_move = 0;
    char del, exit = 0;
    ...
    while ( !exit ) {
        ...
        // detect max moves needed to win the game
        if ( global_max_move < min_move_per_grid ) {
            global_max_move = min_move_per_grid;
            global_result = 0;
            ...
            global_result *= input[ global_max_move - 1 ];
        }
    }
    fclose(f);
