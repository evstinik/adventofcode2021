...
int caves_count = 0, start, end, current_path[MAX_CAVES * MAX_CAVES], path_len = 0, cave_visited_twice = -1;
...
//        print_path();
    ...
    for (int next_cave = 0; next_cave < caves_count; next_cave += 1) {
        ...
        if (
            ...
            || !visited[next_cave]
            || (
                cave_visited_twice == -1
                && next_cave != start
                && next_cave != end
            )
        ) {
            if (is_small_cave(next_cave) && visited[next_cave] && cave_visited_twice == -1) {
                cave_visited_twice = next_cave;
            }
            visited[next_cave] = 1;
            ...
            paths += dfs(next_cave);
            path_len -= 1;
            if (cave_visited_twice == next_cave) {
                cave_visited_twice = -1;
            } else {
                visited[next_cave] = 0;
            }
