...
struct Scanner {
    ...
    Scanner(int number, vector<Point> & points): number(number), points(points), located_to_scanner(-1), center(0, 0, 0), matched_permutation(0) {
...
//                    printf("%d,%d,%d (%d)\n", p1.x, p1.y, p1.z, it->second[i].first);
                ...
                if (valid) {
                    found_matches += 1;
                    _scanner2.located_to_scanner = scanner1.number;
                    _scanner2.center = v;
    ...
    while (scanners.size() > 1 && merged) {
        ...
        for (int i = 0; i < scanners.size(); i += 1) {
            for (int j = i + 1; j < scanners.size(); j += 1) {
                ...
                if ( result ) {
                    printf("Scanners %d and %d can be merged %d ways (used permutation %d)\n", scanners[i].number, scanners[j].number, result, scanners[j].matched_permutation);
                    printf("Scanner %d had %d points, scanner %d had %d points, should be %d points\n", scanners[i].number, scanners[i].points.size(), scanners[j].number, scanners[j].points.size(), scanners[i].points.size() + scanners[j].points.size() - 12);
                    merged = true;
                    // merging
...
//    set<int> located, not_located;
//    for (int i = 0; i < scanners.size(); i += 1) {
//        not_located.insert(i);
//    }
//
//    while (not_located.size() > 1) {
//        auto it = not_located.begin();
//        while ( it != not_located.end() ) {
//            bool merged = false;
//            // try to locate not located to every else we have
//            for (int i = 0; i < scanners.size(); i += 1) {
//                if (i != *it) {
//                    int result = merge_scanners(scanners[i], scanners[*it]);
//                    if ( result ) {
//                        printf("Scanners %d and %d can be merged %d ways\n", scanners[i].number, scanners[*it].number, result);
//                        auto tmp = next(it);
//                        not_located.erase(it);
//                        it = tmp;
//                        merged = true;
//                    }
//                }
